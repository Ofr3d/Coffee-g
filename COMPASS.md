...
- **Provider: Google Gemini, on `gemini-2.5-flash`.** Started on `gemini-2.5-pro` (the capable tier — the hard fork deserves it), but Pro returns `429` on the free tier (it needs billing enabled). Switched to Flash, which has real free-tier quota. The model is a one-line constant in the transport; the engine is untouched — proof the provider-agnostic seam works. Pro stays a one-line flip + billing away if evidence ever shows Flash fumbling a subtle call. Cheap/premium routing still deferred.
- **Tests:** `npm test` → 7 passing unit tests on the engine's contract (`node --test`, zero new deps). Build clean.

**Keystone VERIFIED end-to-end (2026-06-28, on Flash, via `scripts/try-coach.mjs`):** both acceptance cups landed. (1) sour + coarse V60 → *"classic under-extraction… grind significantly finer, ~25–28 clicks"* (concrete fix). (2) sensible in-range espresso, disliked bright acidity → *"brewed correctly… that's the bean's character… try a washed process or different origin"* (it's the bean, not you — no invented fix). The fork is real on the actual model, not just in the prompt contract. Flash handled the subtle preference call cleanly, which is the evidence that Pro/billing isn't needed yet.

**Still to verify on the live phone:** the deployed Netlify site itself — `GEMINI_API_KEY` is set in the dashboard; confirm the in-app "☕ Dial me in" button returns coaching once the Flash deploy is live.
